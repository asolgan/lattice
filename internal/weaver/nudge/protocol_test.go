...
func (failingAdapter) Execute(context.Context, bridge.Request) (bridge.Result, error) {
	return bridge.Result{}, errors.New("external system unavailable")
}
...
func TestNudger_RunAdvancesClaimedExecutingResolved(t *testing.T) {
	ctx := context.Background()
	cs := newClaimStore(t, ctx)
	fake := bridge.NewFakeBackgroundCheck()
	...
	probeFake := bridge.AdapterFunc(func(ctx context.Context, req bridge.Request) (bridge.Result, error) {
	...
	resolveCalls := 0
	resolve := func(ctx context.Context, claimID string, result bridge.Result) (string, error) {
		resolveCalls++
...
func TestNudger_RunMissingAdapterIsConfigError(t *testing.T) {
	ctx := context.Background()
	cs := newClaimStore(t, ctx)
	n := nudge.NewNudger(cs, bridge.NewRegistry())
	_, err := n.Run(ctx, dispatch("c-x"), func(context.Context, string, bridge.Result) (string, error) { return "", nil })
	if err == nil {
...
func TestNudger_RunAdapterFailureLandsFailed(t *testing.T) {
	...
	rec, err := n.Run(ctx, dispatch("claim-fail"), func(context.Context, string, bridge.Result) (string, error) {
...
func TestNudger_RecoverReusesClaimIDNoSecondSideEffect(t *testing.T) {
	...
	rec, err := n.Recover(ctx, d, probe, func(context.Context, string, bridge.Result) (string, error) {
...
func TestNudger_RecoverExecutingReExecutesSameKey(t *testing.T) {
	...
	if _, err := fake.Execute(ctx, bridge.Request{
	...
	resolved := false
	rec, err := n.Recover(ctx, d, probeNoResolve, func(context.Context, string, bridge.Result) (string, error) {
		resolved = true
...
func TestNudger_RecoverRejectsEmptyClaimID(t *testing.T) {
	...
	if _, err := n.Recover(ctx, d, probe, func(context.Context, string, bridge.Result) (string, error) { return "", nil }); err == nil {
...
func TestNudger_RecoverRejectsNilProbe(t *testing.T) {
	...
	cs := newClaimStore(t, ctx)
	reg := bridge.NewRegistry()
	if err := reg.Register("backgroundCheck", bridge.NewFakeBackgroundCheck()); err != nil {
		t.Fatal(err)
	}
	n := nudge.NewNudger(cs, reg)
	d := dispatch("claim-nil-probe")
	if _, err := n.Recover(ctx, d, nil, func(context.Context, string, bridge.Result) (string, error) { return "", nil }); err == nil {
...
func TestNudger_RunRejectsExistingClaim(t *testing.T) {
	...
	fake := bridge.NewFakeBackgroundCheck()
	adapterCalls := 0
	guarded := bridge.AdapterFunc(func(ctx context.Context, req bridge.Request) (bridge.Result, error) {
		adapterCalls++
		return fake.Execute(ctx, req)
	})
	...
	_, err := n.Run(ctx, d, func(context.Context, string, bridge.Result) (string, error) {
...
func (panickingAdapter) Execute(context.Context, bridge.Request) (bridge.Result, error) {
...
func TestNudger_RunContainsAdapterPanic(t *testing.T) {
	...
	rec, err := n.Run(ctx, dispatch("claim-panic"), func(context.Context, string, bridge.Result) (string, error) {
