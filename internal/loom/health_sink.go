...
// consumerHealthEntry is Loom's minimal per-consumer pause-state document,
// stored under health.loom.<instance>.consumer.<name> in the health-kv bucket —
// a SEPARATE, smaller shape from the Contract #5 heartbeat document. It carries
// only the fields HealthSink.Load needs to restore pause state across a restart.
...
func newConsumerHealthSink(conn *substrate.Conn, bucket, instance, name string, states *consumerStateCache) *consumerHealthSink {
	return &consumerHealthSink{
		conn:   conn,
		bucket: bucket,
		key:    "health.loom." + instance + ".consumer." + name,
		name:   name,
		states: states,
	}
}
