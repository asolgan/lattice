...
func TestInferExternalTaskReads(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "subject.<aspect>.data.<field> adds the aspect key as an egress read",
			params:     `{"fullName":"subject.demographics.data.fullName"}`,
			want:       []string{subj},
			wantEgress: []string{subj + ".demographics"},
		},
		{
		...
		},
		{
			...
			want:       []string{subj},
			wantEgress: []string{subj + ".alpha", subj + ".demographics", subj + ".zeta"},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotReads, gotEgress, err := inferExternalTaskReads(subj, raw)
			if tc.errSub != "" {
				if err == nil {
					t.Fatalf("expected error containing %q, got nil (reads=%v egressReads=%v)", tc.errSub, gotReads, gotEgress)
				}
			...
			if !reflect.DeepEqual(gotReads, tc.want) {
				t.Fatalf("reads mismatch:\n got %v\nwant %v", gotReads, tc.want)
			}
			if !reflect.DeepEqual(gotEgress, tc.wantEgress) {
				t.Fatalf("egressReads mismatch:\n got %v\nwant %v", gotEgress, tc.wantEgress)
			}
		})
	}
}
