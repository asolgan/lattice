...
// starlarkDictToGoMap). This is the seam that governs what a DDL script reads
// from a vertex/op payload and what it writes back: a silent regression here
// corrupts script-visible data on the platform's sole writer, so the type
// mappings — especially the JSON-integer preservation trick — are pinned
// directly rather than left to incidental coverage.

// jsonInteger names the subtle invariant: Go decodes every JSON number as
// float64, but a script comparing `x == 5` or indexing must see an integer for
// whole-valued numbers, so an integral float64 marshals to a Starlark Int while
// a fractional one stays a Float.
...
// An Int wider than int64 cannot be represented as a Go int64, so it degrades
// to its decimal string rather than silently truncating to a wrong number.
...
// A Starlark value with no structural Go analogue (here a tuple) is rendered to
// its String() form rather than dropped or panicking.
...
// starlarkDictToGoMap only carries string-keyed entries; a non-string key (legal
// in Starlark, illegal as a JSON object key) is skipped, not coerced.
...
// generic map goes Go -> Starlark -> Go. Strings/bools/nested structure survive
// unchanged; whole-valued JSON numbers come back as int64 (the integer
// preservation trick), fractional ones as float64. Pinning the round-trip guards
// every script that reads a value and writes it back.
