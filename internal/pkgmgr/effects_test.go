...
func TestValidateEffects_MultipleGuardsPerOp(t *testing.T) {
	def := Definition{
		...
		OpMetas: []OpMetaSpec{{OperationType: "DecideLeaseApplication"}},
	}
	if err := def.validateEffects(); err != nil {
...
func TestValidateEffects_OperationTypeHasNoOpMetaSpec(t *testing.T) {
	def := Definition{DDLs: []DDLSpec{{
		CanonicalName:     "leaseapp",
		PermittedCommands: []string{"SignLease"},
		Effects: map[string][]json.RawMessage{
	...
	err := def.validateEffects()
	if err == nil {
		t.Fatal("expected error for an Effects operationType with no matching OpMetaSpec, got nil")
	}
	if !strings.Contains(err.Error(), "SignLease") || !strings.Contains(err.Error(), "OpMetaSpec") {
		t.Errorf("error should name the offending operationType and OpMetaSpec; got %q", err)
	}
}
