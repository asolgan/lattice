...
// sibling of loftspace-app. Like both it is a trusted single-identity tool: it
// connects to NATS as the primordial admin actor and submits operations on the
// user's behalf — there is NO per-user authN/authZ, Gateway, or read-path auth yet
// (Phase-3+). The view is patient-centric: the user selects which patient they are
// and the UI scopes its reads and writes to that patient.
//
