...
// P5) — the only auth concern is minting the staff Bearer token the Gateway's
// write path verifies (real-actor-write-auth-e2e-design.md §3.1's
// shared-dev-IdP posture, mirroring loftspace-app/clinic-app's own
// handleStaffDevToken). Every café op (OpenTab/Charge/Settle) is
// grantsTo:[operator] scope:any, so a single staff token — minted for this
// app's own admin actor — covers every write.
