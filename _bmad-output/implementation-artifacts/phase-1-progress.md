...
- **4.7 carry — legacy bootstrap accessor cleanup**: `internal/bootstrap/{identity_ddl.go,role_mgmt_ddl.go,nanoid.go}` still expose `IdentityPermissions()`, `IdentityGrants()`, `IdentityPermEntry`, `IdentityGrantSpec`, `RoleMgmtPermissions()`, `RoleMgmtGrants()`, `DDLGrantsPermissionID`, `IdentityPermissionKeys()`, `IdentityGrantLinkKeys()` — all dead code (no caller outside the bootstrap package; primordial.go was rewritten to not use them). The `grantsPermission` string literals embedded in these helpers are now stale (link DDL was renamed to `grantedBy` in 4.7). **Drift hazard**: a future developer might copy a key-shape from `IdentityGrantLinkKeys()` and silently break the renamed Capability Lens cypher match. Cleanup pass should delete these surfaces. Kept for now because `bootstrap.IdentityDDL()` + `bootstrap.RoleMgmtDDLs()` (which return only the DDL script body, no perms/grants) are still used by the processor tests as DDL-source-of-truth accessors — those two stay; the others go.
- **4.7 carry — processor test cutover deferred**: brief §5 specified migrating processor tests (identity_create_test, identity_claim_test, identity_state_machine_test, role_mgmt_starlark_test, plus all Story 3.x suites) to install rbac-domain + identity-domain via `InstallPhase1Packages` in test setup. Sub-agent kept the legacy `bootstrap.IdentityDDL()` / `bootstrap.RoleMgmtDDLs()` accessor path instead — tests pass unchanged but exercise a code path that no longer matches the production kernel shape. `internal/testutil/install_phase1_packages.go` is shipped and ready; the cutover is mechanical (add the call after `bootstrap.SeedPrimordial(...)` in each test's setup). Couples with the legacy-accessor cleanup carry above.
- **4.7 carry — `verify-package-*` gates are weak**: targets shell `lattice-pkg list` rather than asserting on per-package Core KV state (DDL aspects, permission vertices, grant link keys, lens specs). Sufficient to detect total-failure-to-install, not sufficient to detect partial-install corruption. Strengthen with per-package assertion scripts (mirror the old `verify-bootstrap.go` style) before Phase 1 closure.
- **4.7 carry — `IdentityDDL()` + `RoleMgmtDDLs()` script-body duplication**: the DDL script bodies are byte-equivalent between `internal/bootstrap/` and `packages/{identity-domain,rbac-domain}/ddls.go` at story close. If they diverge silently, processor tests pass with stale-bootstrap-script while the actual installed-package script does something different. Add a Go test that asserts byte-equivalence (`TestBootstrapDDL_MatchesPackage_Identity`, `TestBootstrapDDL_MatchesPackage_Rbac`) — small follow-up, eliminates the divergence risk while the legacy accessors remain in place.
