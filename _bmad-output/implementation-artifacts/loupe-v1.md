...
- `GET /api/systemmap` — the self-truthing system-map data layer: the canonical component topology (the deployed subset of `architecture-overview.md` — Processor / Refractor / Weaver / Loom / Bridge / object-store-manager + the core-operations / core-events / core-kv spine) with the live Health KV overlay (`computeSystemMap`, `cmd/loupe/systemmap.go`). A declared component with no heartbeat renders `absent` (red); a stale heartbeat `stale` (yellow); each live lens becomes a node parented to Refractor. Returns `{nodes, edges, overall}` — the base layer the planned live system-map landing view + agent-activity console render.
