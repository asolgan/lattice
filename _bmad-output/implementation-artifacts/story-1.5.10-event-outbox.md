...
- **Fork C — republish guard: consumer-idempotency + delete-after-publish (no `published` marker).** Because B is outbox-only there is exactly ONE publisher. The durable consumer's offset is the primary progress guard; after a confirmed `PublishBatch` the consumer **tombstones `vtx.op.<id>.events`** (bounds Core KV growth and, with `history=1`, normally removes the record before a replay would re-read it). This is NOT an exactly-once guarantee: a deliberate seq-0 rebuild, or a publisher retry after a client-side error, may republish — which is safe because events are **at-least-once and consumers idempotent** (Contract #4 §4.4). No extra per-op marker write. (Follow-up, out of this story: optional `Nats-Msg-Id: <eventId>` + a `core-events` dedup window would collapse publisher-retry duplicates at the stream — deferred pending verification that the atomic-publish path honors message dedup.)
