...
**Resolution [Story 2.3, 2026-05-15]:** RESOLVED. `parseCoreKVKey` removed entirely from `internal/refractor/pipeline/pipeline.go`. Both call sites now use `substrate.ParseVertexKey(key)` (returns `(vertexType, id, ok)`). Non-vertex keys are handled per Decision #5: aspect keys log "aspect mutation observed but no handler registered" at INFO; link keys log "link mutation observed but no handler registered" at INFO; unknown-shape keys log "unknown key shape — defect signal" at WARN. All 27 `node_<label>_<id>` fixtures in `pipeline_test.go` migrated to `vtx.<type>.<sentinel-nanoid>` shape with semantic intent preserved. The e2e test (`TestRefractor_E2E_P99`) updated from legacy key shape to `vtx.contract.<NanoID>` — all 100 contracts project correctly at p99=11ms (well within 500ms budget). Observability marker fired during e2e: "aspect mutation observed but no handler registered" for `vtx.meta.RfxE2eP99TestLensXYZ.spec`. Full test suite green (`go test ./... -p 1 -count=1`), `make verify-bootstrap` and `make test-bypass` both pass. Story 3.2 (Capability Lens activation) is now unblocked.
