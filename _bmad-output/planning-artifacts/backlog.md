...
| Loupe live system-map *(★★★ experience)* — **FE landing view** | 🏗️ Next (UX-then-FE: Sally designs → FE Engineer builds + verifies in-browser; renders `/api/systemmap`) | Refinements & ops table |
...
| Loupe live "system map" landing page *(Loupe)* | Landing view renders the running component + data-flow topology (the `architecture-overview` shape, deployed subset) as a **live** diagram — per-component/lens Health indicators, edge/link status, drill-in to vertices and control planes. Self-truthing: generated from Health KV + Core KV, not a static image. Base layer for the planned agent-activity operator console (`implementation-artifacts/agentic-ops-design.md`). **Data layer shipped** — `GET /api/systemmap` (`computeSystemMap`) composes the `{nodes, edges, overall}` topology with the live Health overlay; **remaining = the FE landing view** (UX-then-FE: Sally designs → FE Engineer builds + verifies in-browser). | ★★★ | M (FE remaining) |
