...
| Conventions-linter follow-ups *(agentic-ops)* | `make lint-conventions` (`scripts/lint-conventions.go`) ships advisory and flagged **24 pre-existing `// Story N …` history-comments**. Follow-ups: (a) sweep those to present-tense; (b) then wire `lint-conventions` as a changed-file `PostToolUse` hook and flip `STRICT=1` as a CI gate (tree must be clean first). | ★ | S |
