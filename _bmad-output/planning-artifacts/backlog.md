...
| LoftSpace applicant app — **FE Increment A: scaffold + Browse&Apply** | ✅ Done — new `cmd/loftspace-app` (binds `127.0.0.1:7788`, trusted admin actor like Loupe): `GET /api/listings` (new tombstone-filtering listing+address assembler, unit-tested) + `GET /api/identities` (applicant switcher) + `POST /api/op` (submit), and the vanilla Browse&Apply FE — listings grid, identity switcher (localStorage-persisted), DDL-aligned apply form submitting `CreateLeaseApplication`. **In-browser verified** against the live stack (browse → apply → real leaseapp committed, green toast); a modal-on-load `[hidden]`-override bug was caught in-browser + fixed. `make run-loftspace-app` added. All gates green; lead-reviewed. **⚠️ Known P5 deviation (Andrew, 2026-06-25):** `/api/listings` + `/api/identities` read by scanning Core KV — a violation of architecture **P5** ("lenses are the only application query surface"; Loupe-the-inspector is the only Core-KV-scan exception). Corrective increment **A.1** queued below. Increments B (status) → C (tasks) → D (documents) → E (up-full wiring) next. | `cmd/loftspace-app/` |
| LoftSpace applicant app — **FE Increment A.1: repoint reads to lens projections (P5 fix)** | 📋 Queued — replace the Core-KV scans with read-model reads. (1) Add a **listings lens** (loftspace-domain) projecting available `vtx.unit` + `.listing` + `.address` → a NATS-KV target bucket; `/api/listings` reads that bucket. (2) `/api/applications` (Increment B) already reads the `leaseApplicationComplete` projection (`weaver-targets`) — keep. (3) Identities switcher → a roster lens read model (lower priority). Closes the P5 deviation in Increment A. | `implementation-artifacts/loftspace-applicant-app-ux.md` §4.1/§6 |
