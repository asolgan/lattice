...
| **objects-base v1b GC — concurrent-attach reclaim race** | 📋 Bug (★★, M — reliability; needs GC convergence change) — **Verified live** while shipping Increment D: freshly-attached objects are GC-reclaimed (3/5 under rapid/concurrent uploads; 5 `TombstoneObject` fired seconds after attach; a lone *unhurried* upload survives). **Root cause:** `objectLiveness` detects orphans from the **adjacency** projection, which lags the *atomic* `AttachObject` commit → momentary `liveOwners=0` → Weaver dispatches `TombstoneObject`. The v1b design's **CC1** (`large-file-binary-design.md:388`) anticipated exactly this ("'0 edges in adjacency' ≠ '0 live links' — trusting it reaps freshly-attached objects") and mandated an authoritative Core-KV live-link scan, but the shipped Option A (lens + epoch-CAS) closes only the *re-link* race, not this *attach-adjacency-lag* race (the epoch is already bumped atomically → CAS matches → tombstone proceeds). Affects **all** object uploads incl. Loupe's Files tab. **Fix direction:** make `objectLiveness` liveness consistent with `linkEpoch` (root-data live-link count maintained atomically by Attach/Detach, replacing the adjacency count) OR implement CC1's authoritative Core-KV link scan in the `TombstoneObject` guard. Own fire — GC convergence + heavy `make test-object-gc` e2e + 3-layer review. | `packages/objects-base/{lenses.go,ddls.go}`, `internal/weaver`, `_bmad-output/implementation-artifacts/large-file-binary-design.md` §20/CC1 |
