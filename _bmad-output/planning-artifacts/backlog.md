...
| Loupe task-inbox — **data layer** (`GET /api/tasks`) | ✅ Done (link-sourced tasks + op label resolved from forOperation meta; the Winston "resolve-from-link, don't stamp-aspect" call; unit-tested, gates green) | `cmd/loupe/tasks.go` |
...
| Human-readable task content on userTasks / assignTasks | **Re-scoped 2026-06-25 (Winston) — NOT a platform/package gap; the content is reachable today.** Task *relationships are links* by design (Contract #10 §10.1): the `my-tasks` lens already projects `forOperation: vtx.meta.<id>`, and that op meta-vertex carries `.canonicalName` + the DDL self-description (inputSchema / fieldDescription / examples) Loupe **already reads to render op-submit forms**. So a task-inbox renders {op canonicalName = title, op description = instructions, op inputSchema = the form} by resolving the `forOperation` meta — the same Core-KV meta-read pattern Loupe's `resolveLens` already ships. Decision: **resolve-from-link, do NOT stamp a `.prompt` aspect** (single source of truth; no dual-write; preserves the task root's deliberate `{status,expiresAt}`-only / NO-aspects invariant; no §10.8 dispatch-`reads` / contract touch). Residual is **FE-only** (a task-inbox surface) + a minor optional package-authoring nicety (ensure each userTask op carries a human `description` aspect, not just the machine `canonicalName`). ✅ **Loupe task-inbox data layer done** (`GET /api/tasks`: lists tasks, link-sources assignee/op/target, resolves the op's human label from its meta; unit-tested, gates green). Residual: the FE tab (UX-then-FE) + the optional op-`description` authoring nicety. | LoftSpace (all) | FE | ★★ | S |
