...
Trackers are written with a **24-hour per-key TTL** at commit step 8, using NATS JetStream's per-message TTL feature (ADR-48, NATS 2.11+). After 24 hours, NATS publishes a `PURGE` marker for the tracker's key with header `Nats-Marker-Reason: MaxAge`, which Refractor and other CDC consumers observe as an explicit expiry event.
