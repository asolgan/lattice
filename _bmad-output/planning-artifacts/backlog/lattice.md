...
| **repo-debris-and-stale-narration** | Remove the five resolved CONTRACT-AMENDMENT-REQUEST.md journals (cmd/{loom,processor,refractor,weaver}, internal/substrate — git is the record); fix the objects-base OpMetas comments (ddls.go/permissions.go) calling the reclaim a nonexistent "reclaimObject Loom pattern" (it's the objmgr §22 owner-cascade); add the missing BOOTSTRAP_JSON_PATH to the objmgr up-full launch (Makefile); decide internal/spike disposition. | ★ | S | 📋 |
