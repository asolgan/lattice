...
| **[Weaver] Heartbeat status/issue-severity inconsistency** | Live-observed during the Loupe status/issues work: Weaver emits `status:"healthy"` while carrying `severity:"error"` issues (`TemplateDataError` — target `leaseApplicationComplete` gap, param `subject` references `row.applicant` which is null/absent). Contract #5 §5.3 couples an `error` issue to `unhealthy` (and a `warning` to `degraded`); a `healthy` status with `error` issues is self-inconsistent. Decide per case: either set `status:"degraded"`/`"unhealthy"` to match, **or** downgrade a recoverable per-target data gap to `severity:"warning"` (degraded). Loupe now escalates the *rendered* severity off the worst issue regardless (so the operator sees it), but the source emission should be made consistent. Ref: `internal/weaver` heartbeat construction; the gap itself is a real LoftSpace data issue (applicant link absent on the row). | ★★ | S | 📋 ready |
