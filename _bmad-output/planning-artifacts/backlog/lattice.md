...
| **[auth] operator role is class-blind all-or-nothing kernel root** | Any `holdsRole→operator` identity (incl. a plain-`identity` human) gets the full anchor — all privileged lanes + kernel ops; no scoped-operator tier; drifts vs control-plane §3.5 "ordinary actor"; boot-snapshot staleness. Fix B: scoped `consoleOperator`; C: per-role meta-lane grants. | ★★ | M | 🔭 flag-for-Andrew · [design](../../implementation-artifacts/loupe-operator-auth-lift-design.md) §4 · Lattice half of the Loupe lift |
