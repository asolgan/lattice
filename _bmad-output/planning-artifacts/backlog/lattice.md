...
| **[auth] scoped privileged-lane grants (retire all-or-nothing operator-root)** | `holdsRole→operator` is class-blind full root — no middle tier; a Loupe operator can't run pkg-install without being kernel root; boot-snapshot staleness. Fix: per-op lanes in `cap.roles` + a core allowlist (C1) → a `consoleOperator` runs meta-lane pkg-lifecycle without root. | ★★ | M | ✅ direction ratified (Andrew, C after B) · 📐 mechanism awaiting-Andrew · [design](../../implementation-artifacts/scoped-privileged-lane-grants-design.md) |
