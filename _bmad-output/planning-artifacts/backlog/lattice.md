...
| **[service-location] `Wire*` cannot revive a tombstoned link** | The shared `wire()` helper emits `op:"create"` for a tombstoned link; create asserts revision 0 ⇒ `RevisionConflict`. Every Wire op inherits it, so `residesIn` is affected: a resident who moves out can never be moved back in. Fix is `update` semantics for the revive branch, plus declaring the link key as a read at the Wire callers (today only the endpoints are declared). | ★★ | S | 📋 ready · found by staff-worlds F3 |
