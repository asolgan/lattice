...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **protected-flag-create-guard-vector** | Root is designated two ways that disagree: Contract #7 §7.7 says `holdsRole→operator` topology (self-protecting); shipped core drifted to the forgeable `data.protected` bit (Epic-12) — read wildcard unconditionally forgeable. Re-converge core on the topology + retire the bit as designator; create-guard + Gate-2/3 is the interim floor. Epic-12 obligation. | ★★★ | S–M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/root-identity-designation-design.md) · fork: topology (rec.) vs harden-bit; §7.7 drift |
