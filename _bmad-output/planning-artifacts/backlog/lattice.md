...
| **[Core] `packages/privacy-base` missing `manifest.yaml`** | Every other package ships a `manifest.yaml` (`lattice-pkg install` reads it to resolve + verify the compiled `Definition`); privacy-base never got one (Fire 1, pre-dates this finding), so `make up-full`/CLI install fails with `open packages/privacy-base/manifest.yaml: no such file`. Go-level installs (`pkgmgr.Installer.Install`, used by all tests) are unaffected. Found 2026-07-02 blocking Fire 4a's stack bring-up. | ★ | XS | 📋 |
