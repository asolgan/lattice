...
| **real-actor-write-auth-e2e** | Prove scoped capability write-auth end-to-end: apps submit as real role-scoped users through the Gateway (not `bootstrap` root) via a shared dev Fake IdP, under `up-full-capability`, with a genuine allow + deny. Retires the stub as a load-bearing crutch (app tier; system-actor Fire 2 did the engine tier). Browser-direct topology. | ★★★ | L | ✅ Phase 1 all done (items 1-6) · [design](../../implementation-artifacts/real-actor-write-auth-e2e-design.md) · Phase 2 (walk-in credential binding) remains |
