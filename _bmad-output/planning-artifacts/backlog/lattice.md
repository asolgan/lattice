...
| **[Bridge/Processor] Op-status read surface — `lattice.op.status` responder** | Processor-hosted op-status RPC (vault.decrypt pattern); Fire 4: CLI — the gate for matrix-wide read tightening. | ★★★ | S | 🏗️ building · [design](../../implementation-artifacts/op-status-read-surface-design.md) · §10.6 contract edit UNCOMMITTED for Andrew · next: Fire 4 CLI |
