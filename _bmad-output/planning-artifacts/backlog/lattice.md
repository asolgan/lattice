...
| **Read-posture debt sweep (platform packages) → flip lint to blocking** | 21 class-(b) lazy `kv.Read` sites in `packages/` — augur 8 · capability-author 7 · orchestration-base 3 · identity-domain 2 · privacy-base 1 — declare (a/d) or annotate (c/e) incl. dispatcher envelopes; add the missing `(a)` annotation form; flip the findings advisory→blocking after the [verticals half](verticals.md) — stops accrual (65 today), feeds Edge's declared-reads ⊆ mirror gate. | ★★★ | S–M | 📋 ready · [design](../../implementation-artifacts/script-read-posture-design.md) §12 |
