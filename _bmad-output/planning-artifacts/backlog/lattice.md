...
| **[Core] Health rollup's `classifyKey` default-buckets 9 real heartbeats as "lens"** | gateway/bridge/objmgr/chronicler/vault/4 vertical apps emit a `status:"healthy"`+`heartbeatAt` heartbeat, but `classifyKey` has no case for them — falls to `default:"lens"`, which only knows `status ∈ {active,paused,rebuilding}`+`lastUpdated`. All 9 read `"unknown"` forever, healthy or dead — confirmed live, a gateway heartbeat frozen ~50h reads identical to a fresh one. Add heartbeat cases mirroring weaver/loom's. | ★★★ | S | 📋 ready · `health.go:89-115,290-330` |
