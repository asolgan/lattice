...
| **Edge gap-detection needs STREAM.INFO, which the grant denies** | `sync.Manager.gapped()` → `FirstSequence` → `$JS.API.STREAM.INFO.SYNC`, omitted from the Edge callout grant (leaks cross-identity stream stats). Proven denied under the real grant (W3-inc-3b harness); hits the shipped Go nodes AND the browser node — warm resume w/ a stored cursor fails closed (cold start hydrates, so latent). Fix = design call (scoped STREAM.INFO, or gap from CONSUMER.INFO), not a steward grant-widening. | ★★ | M | 📋 needs-design → Designer (latent; not a trust-boundary fork) |
