...
| **Continuous security-proof watchdog (new component)** | Standing red-team: re-runs the bypass (g2) + capability (g3) adversarial proofs in an **isolated ephemeral stack** (never the live stack), scheduled + on-demand → pass+freshness to Health KV. Supersedes the destructive `make test-bypass`/`-capability-adversarial` (down+up kernel-only nukes a live up-full stack). Loupe gate chips removed for now. | ★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/security-proof-watchdog-design.md) · Warden (2 forks) |
