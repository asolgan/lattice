...
| **[Core] UninstallPackage tombstones unconditionally (F-011 per-key OCC follow-up)** | `Upgrade` still submits without per-key `expectedRevision` — a concurrent write to a declared key is silently overwritten. Fix: condition on the read-time `KVGet` revision (already read). | ★ | S | ✅ ratified · [design](../../implementation-artifacts/package-install-per-key-occ-design.md) · Fire 1 (uninstall) shipped `1a9b2e5`; next: Fire 2 (upgrade) |
