...
| **real-actor-write-auth-e2e** | Prove scoped capability write-auth end-to-end: apps submit as real role-scoped users through the Gateway (not `bootstrap` root) via a shared dev Fake IdP, under `up-full-capability`, with a genuine allow + deny. Retires the stub as a load-bearing crutch (app tier; system-actor Fire 2 did the engine tier). Browser-direct topology. | ★★★ | L | 🏗️ building · [design](../../implementation-artifacts/real-actor-write-auth-e2e-design.md) · Phase 1 items 1+2 shipped (`cf102b4`/`7326774`); next: item 3 (up-full-capability) + item 4 (e2e) |
