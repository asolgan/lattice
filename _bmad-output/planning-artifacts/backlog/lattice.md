...
| **[bootstrap] Stale `lattice.bootstrap.json` vs. a recreated Core KV silently breaks reads** | `make up`'s reuse branch skips re-bootstrapping when kernel processes are already running, so a container recreated outside `make down` leaves the JSON stale — reads return empty while writes still succeed. Recurred 3× (2026-07-03/04): now proven to also break a LIVE vertical (Clinic staff-wildcard RLS view stays empty — the app's admin actor is the stale identity, so it holds no live WildcardAnchor grant — blocking the Book UI's patient picker end-to-end; self-anchored patient/provider reads are unaffected). | ★★ | S | 📋 · repro: `lattice bootstrap verify`; live repro: create a Clinic patient then GET `/api/staff/patients` → count 0 despite the row existing · fix: freshness check on the reuse path, or a Health-KV signal |
