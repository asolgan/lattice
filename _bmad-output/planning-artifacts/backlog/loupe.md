...
| **F20 — Hosted-demo read-only operator** | Andrew wants Loupe in the public demo as the behind-the-scenes view: a `demoOperator` role stripped to inspect-only grants — every write surface (cap apply, Augur dispatch, vault reveal) capability-denied at the platform, proving "even the console is capability-scoped" live; one-tap demo login mirroring Facet's persona posture; own subdomain + visitor disclaimer. Exposed only at the demo's public-launch phase. | ★★ | M | 🚧 needs design (role scoping + exposure posture) |
