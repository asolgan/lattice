...
- **Next:** **F16.1** (capability see+reject) is the top buildable fire — ★★★, zero cross-lane dep. Then F16.3 (Augur, zero dep, first prod `ReviewProposal` use), then F16.2 (cap approve+apply, two contingent spikes). The `designAhead`-trio maintenance row is 📋 ready (smaller). F13 L2-full/L3 still blocked on Chronicler archive mode (unscheduled, lattice).
