...
| **Display names across the graph ("fewer NanoIDs")** | Andrew PO pass: raw NanoIDs are the primary label across Facet (header, Unit/Building chips, leaseapp tasks, "Unnamed" Me). Four display classes — metas=`canonicalName` · business=`.presentation` · identity=sealed PII (self via EDGE.4 vault client; others via Protected lenses) · relational=lens-projected context — + the floor rule: a bare NanoID is never a primary label. | Cross-vertical | FE + pkg | ★★★ | M–L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/display-name-convention-design.md) |
