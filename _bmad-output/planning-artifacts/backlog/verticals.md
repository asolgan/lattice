...
| **No patient self-service booking** | Patients only ever *view* (`My Appointments`/`My Schedule`, D1.5 self-RLS reads) — every `CreateAppointment` is staff-initiated via the front-desk patient-select dropdown; there is no patient-authenticated write path. Real scheduling products (Zocdoc-shaped) let a patient book their own slot. `CreateAppointment` already exists; add a self-scoped write path gated to `patient == caller`, mirroring the shipped LoftSpace consumer-scope self-write precedent (2026-07-07 Done log). | Clinic | pkg + FE | ★★★ | M | 📋 ready |
