...
| Clinic — booking constraint as write-path slot claims (15-min grid) | Double-book enforcement moves from read-enumeration (`kv.Links` over the banner-withdrawn `hasBooking` links) to create-only per-slot claim keys in the booking batch (mirrors email-uniqueness/`appliedToUnit`): overlap = key collision = typed reject; cancel tombstones claims, rebook restores. Drops `hasBooking` (§1.1 restored), retires the `bookingGuard` epoch. Grid = package constraint (Andrew, 2026-07-02). | Clinic | pkg | ★★ | M | 🏗️ designing (Winston) · design doc next |
