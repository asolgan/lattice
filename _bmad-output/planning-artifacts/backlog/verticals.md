...
| **Facet has no browse surface for bookable entities** | Wellness "Book a class" and its siblings render disabled rather than bookable: an op that targets a session or provider needs `ctx.entityKey`, and Facet has no browse surface on which to pick that entity. Needs a session/provider list the renderer can dispatch from. | Facet | FE + pkg | ★★ | M | 📋 ready |
| **Facet café OpenTab can never reopen a settled lease's tab** | Settle soft-deletes the lease's `.cafeOpenTab` guard (`isDeleted:true`), but OpenTab claims it create-only (`expected=0`), so every later OpenTab for that lease dies `RevisionConflict` at the substrate. Same create-only-vs-second-write class the SetLocationPresentation fix already hit. Fix: claim conditioned on the guard being absent-or-tombstoned, preserving one-open-tab-per-lease. | Café | pkg | ★★ | S | 📋 ready |
