...
|---|---|---|---|---|---|---|
| Recurring `@every` schedules — the clinic forcing function | `@every` has no package consumer yet (appointment-reminders + follow-ups already ship on `@at` one-shot). A genuinely-recurring clinic need (e.g. recurring availability / a recurring visit series). | Clinic | platform + pkg | ★★★ | M | 🚧 needs-design · the ratified [`@every`](lattice.md) design delivers only the substrate primitive (Fire 1 ✅ `ScheduleEvery`) + the *internal* Weaver-sweep migration (Fire 2) — NO package-facing recurring seam; a clinic consumer needs a design call (package `@at`-rolling-deadline pattern vs a new Weaver package-recurring lane = lattice), not a blind build. NOT build-ready as filed (steward-verticals 2026-06-30). |
