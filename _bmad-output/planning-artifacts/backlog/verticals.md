...
| **No patient self-service booking** | Patients only ever *view* (My Appointments/My Schedule, D1.5 self-RLS reads); every `CreateAppointment` is staff-initiated. Real scheduling products let a patient book their own slot — add a self-scoped write path gated to `patient == caller`. | Clinic | pkg + FE | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/clinic-patient-self-service-booking-design.md) · next: FE (own fire) |
