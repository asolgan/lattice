...
| **Facet has no browse surface for bookable entities** | Wellness "Book a class" and its siblings render disabled rather than bookable: an op that targets a session or provider needs `ctx.entityKey`, and Facet has no browse surface on which to pick that entity. Needs a session/provider list the renderer can dispatch from. | Facet | FE + pkg | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/facet-entity-browse-design.md) · `availableAt` is authZ-bearing, so reachability needs a `studio locatedAt location` link; build order §4 |
