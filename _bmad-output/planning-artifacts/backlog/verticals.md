...
| LoftSpace — lease renewal (no renewal surface exists; first goal-authored Weaver target) | A lease near expiry needs a per-tenant chain: fresh bgcheck only if stale, guarantor re-verify only if present, rent-adjustment approval, renewal signature. Author it goal-first (one `goal` + op `effects`) instead of gated gap columns — the real consumer that un-holds planner Fire 6 Inc3. | LoftSpace | pkg + FE | ★★★ | L | 📋 needs-design (Designer; Andrew-routed 2026-07-05) · design must incl. the §10.8 goal-first-authoring doctrine rider · [planner design](../../implementation-artifacts/weaver-planner-mandate-design.md) |
