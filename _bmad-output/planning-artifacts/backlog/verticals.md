...
| **Display names across the graph ("fewer NanoIDs")** | Andrew PO pass: raw NanoIDs were the primary label across Facet (header, Unit/Building chips, leaseapp tasks, "Unnamed" Me). Four display classes + the floor rule "a bare NanoID is never a primary label" — see design. | Cross-vertical | FE + pkg | ★★★ | M–L | 🏗️ building · [design](../../implementation-artifacts/display-name-convention-design.md) · N1 + N2 (renderer floor rule + scoped-target name lens, task rows read "Unit 1 lease") shipped; next N3 self-name (sealed `name` decrypt via edge/vault.Client) |
