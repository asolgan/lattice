...
| **Apply to lease is broken for every applicant** | Live-verified: the demo skips the real claim ceremony (`app.js:565`), self-submitting as U's own key, which never gets the `consumer` role — every Apply 403s. Fix is app-side: wire the already-ratified claim-link handoff (§11.1a). Clinic's self-book has the same shape. | LoftSpace | FE + pkg | ★★★ | M | 📋 ready · [design](../../implementation-artifacts/gateway-claim-flow-identity-provisioning-design.md) §11.1 |
