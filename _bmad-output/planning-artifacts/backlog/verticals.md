...
|---|---|---|---|---|---|---|
| **Edge showcase app (Facet)** | Discovery-driven personal client on the Edge foundation: hardcodes only IdP login + connect; services, ops, forms, tasks arrive as data via `edge-manifest` personal lenses + a descriptor vocabulary (#52/#54/#55). PWA-first. | Cross-vertical | Sally + FE Engineer + pkg | ★★★ | XL | 🏗️ building · [design §7.5](../../implementation-artifacts/edge-showcase-app-design.md) · next: wellness/café self-scope-op adoption (wellness row below) or 2nd-renderer spike |
| **Wellness self-service booking** | Residents book/cancel their own class instead of staff submitting for them — mirrors clinic-domain's self-service pattern, and is the precondition Facet Fire 5 named for wellness adoption. Package layer (`CreateBooking`/`CancelBooking` consumer scope=self) shipped + tested headlessly, `ac21a6c`. | Wellness | pkg (FE next) | ★★ | M | 🏗️ building · next: wellness-app FE self-service session (claim ceremony + dev-token minter, mirrors clinic-app's `ensureClaimedDevice`/`selfWriteToken`) — no per-resident login exists in wellness-app today |
