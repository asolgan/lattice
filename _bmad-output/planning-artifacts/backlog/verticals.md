...
|---|---|---|---|---|---|---|
| **Edge showcase app (Facet)** | Discovery-driven personal client on the Edge foundation: hardcodes only IdP login + connect; services, ops, forms, tasks arrive as data via `edge-manifest` personal lenses + a descriptor vocabulary (#52/#54/#55). PWA-first. | Cross-vertical | Sally + FE Engineer + pkg | ★★★ | XL | 🏗️ building · [design §7.2](../../implementation-artifacts/edge-showcase-app-design.md) · next: Inc 2 dev-login sessions — two-browser multi-user green bar |
| **Showcase dataset (curated demo world)** | Checked-in declarative demo dataset + idempotent loader (ops via Gateway, deterministic handles): two personas sharing one building, services with correct families (widens service-domain's closed `SERVICE_FAMILIES` enum), wiring, tasks. Supersedes `seed-edge-demo`'s improvised topology and retires its two backgroundCheck-classed "laundry" templates. | Cross-vertical | pkg | ★★★ | M | 📋 ready · [design §7.3](../../implementation-artifacts/edge-showcase-app-design.md) · feeds Facet Fire 3 Inc 2's two-persona green bar |
