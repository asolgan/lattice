...
| **Facet café OpenTab asks the user to paste their own leaseAppKey** | Live-verified in-browser: the "Open a house tab" descriptor form renders `leaseAppKey` as a required free-text field, but the signed-in identity's own leaseapp is already in its manifest (the Sign Lease task names it). Auto-fill self-anchored keys the manifest already carries instead of asking the visitor to paste a vertex key. | Facet | FE | ★★ | S | 📋 ready |
