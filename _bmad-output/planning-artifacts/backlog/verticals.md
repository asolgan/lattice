...
| **Clinic front-desk follow-up-series grant** | `StartVisitSeries` (Follow-ups tab) is `operator`-only in the **clinic-reminders** package (separate version + confinement helpers from clinic-domain); a real `frontOfHouse` session gets `AuthDenied`. Add the `frontOfHouse` grant + workplace confinement (mirror clinic-domain's `CreateAppointment`), consumer: front-desk Follow-ups tab. | Clinic | pkg | ★★ | S | 🏗️ building · [brief §7](../../implementation-artifacts/clinic-recurring-visit-series-design.md) · grant+confinement on Start/Pause/Resume, operator-exempt guard |
