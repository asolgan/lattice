...
| **Vertical Package Standard — canonicalize the reference corpus** | Write THE standard a vertical package builds to, knowing everything 2026-07 knows: persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it; per-package fires decide converge-vs-rewrite against it. | Cross-vertical | pkg | ★★★ | L | 📋 ready · Designer next (standard doc → 📐 Andrew) |
