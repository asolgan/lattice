...
| **Read-posture debt sweep (vertical packages)** | 44 class-(b) lazy `kv.Read` sites (lease-signing 23 · wellness 10 · clinic 8 · loftspace 3; list = `lint-conventions`). §3.1 rule: required key → `reads` (a) · dedup → `optionalReads` (d) · else annotate `(c)/(e)`; declare at every dispatcher. Platform half (21) + warn→block flip: [lattice.md](lattice.md). | Cross-vertical | pkg | ★★★ | M | 🏗️ building (clinic-domain 5 residual sites left) · [design §13](../../implementation-artifacts/script-read-posture-design.md) |
