...
- **Task/service business-data placement (root `data` vs aspect)** — tasks and services currently keep business fields on the vertex root `data` (e.g. capability cypher reads `task.data.expiresAt`, `svc.class`), consistent with the deliberate permissions exception, but unlike the general model where business data lives in aspects and vertices mostly walk links. No task/service DDL exists yet. Decide aspect-vs-root deliberately when the first task/service DDL is authored; the capability lens cypher (`internal/bootstrap/lenses.go`) must move in lockstep with that decision.
